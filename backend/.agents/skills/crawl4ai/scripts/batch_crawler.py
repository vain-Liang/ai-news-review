...
async def crawl_batch(urls: List[str], max_concurrent: int = 5):
    ...
    browser_config = BrowserConfig(headless=True, viewport_width=1280, viewport_height=800, verbose=False)
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        batch_results = await crawler.arun_many(urls=urls, config=crawler_config, max_concurrent=max_concurrent)
    ...
    # Save results
    output = {"success_count": len(results), "failed_count": len(failed), "results": results, "failed": failed}
...
async def crawl_with_extraction(urls: List[str], schema_file: str = None):
    ...
    crawler_config = CrawlerRunConfig(extraction_strategy=extraction_strategy, cache_mode=CacheMode.BYPASS)
        ...
        for result in results:
            if result.success and result.extracted_content:
                try:
                    ...
                    extracted_data.append({"url": result.url, "data": data})
